...
export function CreatePostForm() {
  ...
  const [cafeId, setCafeId] = useState(mapCafes[0]?.id ?? "");
  ...
  const preview = useMemo((): CommunityPost => {
    const cafe = mapCafes.find((c) => c.id === cafeId) ?? mapCafes[0]!;
    return {
      id: "preview",
      authorName: "أنت",
      authorKind: "customer",
  ...
  }, [kind, cafeId, product, table, body, tags, rewardLink, media]);
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-2">
      <form
        ...
        onSubmit={(e) => {
          e.preventDefault();
          window.alert("تم حفظ المنشور محليًا (واجهة فقط) — الربط بـ API لاحقًا.");
        }}
      >
        <div>
        ...
        <div className="flex flex-wrap gap-3">
          <span className="text-xs font-extrabold text-riwaq-muted">وسائط (وهمي)</span>
        ...
        <button
          type="submit"
          className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-riwaq-brown text-sm font-extrabold text-white shadow-lg hover:bg-riwaq-brown/90"
        >
